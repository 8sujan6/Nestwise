...
Clone the codebase locally using Git:
```bash
git clone https://github.com/8sujan6/Nestwise.git nestwise
cd nestwise
```
...
```
*(No other variables are required; the system defaults to the stable `gemini-3.1-flash-lite` model).*
...
npm run dev
```
Open [http://localhost:3000](http://localhost:3000) in your browser to start searching properties.
